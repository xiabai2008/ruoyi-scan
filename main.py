...
from core.runner import (
    run_mode, run_mode_batch, final_prompt, run_chain_mode, run_serve_mode,
    run_passive_mode, run_template_list_mode, run_diff_only_mode,
...
def build_parser():
    ...
    parser.add_argument('--proxy-rotate', choices=['round-robin', 'random', 'least-fail'], default='round-robin', help='代理轮换策略')
    ...
    parser.add_argument('--timeout', type=int, default=settings.TIMEOUT, help=f'请求超时秒数（默认 {settings.TIMEOUT}s）')
    ...
    parser.add_argument('--ci-init', default=None, metavar='PLATFORM', choices=['github', 'gitlab', 'jenkins'], help='生成 CI 配置')
    # D29 知识库
    ...
    # D36 分布式
    parser.add_argument('--distributed', default=None, metavar='MODE', choices=['master', 'worker', 'standalone'], help='分布式模式')
