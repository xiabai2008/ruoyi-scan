...
class TestFingerprintCache(unittest.TestCase):
    ...
    def test_cache_hit(self):
        ...
        self.assertEqual(count_after_second, count_after_first, "第二次 get 不应增加请求计数（缓存命中）")
...
class TestDetectCmsCaching(unittest.TestCase):
    ...
    def test_detect_cms_caches_root_request(self):
        ...
        with requests_mock.Mocker() as m:
            ...
            favicon_hits = [r for r in m.request_history if r.url == target + "favicon.ico"]
        # 注册 CMS 数（用于校验去重效果）
        n_cms = len(list_cms())
        ...
        self.assertEqual(
            len(root_hits), 1, "根路径应只请求 1 次（缓存去重），实际 %d 次（%d CMS）" % (len(root_hits), n_cms)
        )
        self.assertEqual(
            len(favicon_hits), 1, "favicon 应只请求 1 次（缓存去重），实际 %d 次（%d CMS）" % (len(favicon_hits), n_cms)
        )
