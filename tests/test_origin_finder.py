...
def test_find_origin_ip_returns_list():
    ...
    finder = OriginIPFinder()
    # mock 所有探测方法返回空
    with patch.object(finder, "_check_response_headers", return_value=[]), patch.object(
        finder, "_check_subdomains", return_value=[]
    ), patch.object(finder, "_check_ssl_san", return_value=[]), patch(
        "socket.gethostbyname", side_effect=Exception("mock")
    ):
...
def test_find_origin_ip_with_header_leak():
    ...
    with patch.object(finder, "_check_subdomains", return_value=[]), patch.object(
        finder, "_check_ssl_san", return_value=[]
