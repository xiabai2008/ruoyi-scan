...
class TestCSVReportD18D24:
    ...
    def test_csv_header_contains_fix_detail(self):
        """CSV 表头含"修复详情"列"""
        rb = ReportBuilder(results=_sample_results(), target="http://x/", summary={"target": "http://x/"})
        csv_text = rb.to_csv()
...
class TestHTMLReportD18D24:
    ...
    def test_html_colspan_updated(self):
        ...
        rb = ReportBuilder(results=[], target="http://x/", summary={"target": "http://x/"})
        html = rb.to_html()
...
def _load_all_plugins():
    ...
    plugins = []
    for package_name in discover_plugin_packages():
        try:
...
class TestAllPOCFilledD18D24:
    ...
    def test_vuln_plugins_have_fix_detail(self):
        ...
        for module_name, name, cls in plugins:
            ...
            if getattr(instance, "category", "") == "vuln":
                assert instance.fix_detail, f"{module_name}.{name}（vuln）fix_detail 不应为空"
    ...
    def test_vuln_plugins_have_reproduce(self):
        ...
        for module_name, name, cls in plugins:
            instance = cls()
            if getattr(instance, "category", "") == "vuln":
                assert instance.reproduce, f"{module_name}.{name}（vuln）reproduce 不应为空"
    ...
    def test_fix_detail_contains_upgrade_hint(self):
        ...
        for module_name, name, cls in plugins:
            instance = cls()
            if getattr(instance, "category", "") == "vuln" and getattr(instance, "severity", "") == "high":
                high_vuln_count += 1
                # 高危漏洞的 fix_detail 应含具体修复方向
                assert (
                    "升级" in instance.fix_detail or "配置" in instance.fix_detail or "代码" in instance.fix_detail
    ...
    def test_fix_detail_contains_compliance_mapping(self):
        ...
        for module_name, name, cls in plugins:
            instance = cls()
            if getattr(instance, "category", "") == "vuln" and getattr(instance, "severity", "") in ("high", "medium"):
                assert "OWASP" in instance.fix_detail or "等保" in instance.fix_detail, (
...
class TestEndToEndD18D24:
    ...
    def test_plugin_to_report_flow(self):
        ...
        # 构建报告
        rb = ReportBuilder(results=[result], target="http://target/", summary={"target": "http://target/"})
    ...
    def test_dedup_preserves_d18_d24(self):
        ...
        assert len(aggregated) == 1
        # 渲染为报告
        rb = ReportBuilder(results=aggregated, target="http://x/", summary={"target": "http://x/"})
        csv_text = rb.to_csv()
