...
class TestCrawler:
    ...
    def test_crawl_filters_static_resources(self):
        ...
        sess = FakeSession(
            {
                "http://x.com/": FakeResp('<a href="/img.png">IMG</a><a href="/page">P</a>', 200),
    ...
    def test_crawl_with_js_urls(self):
        ...
        sess = FakeSession(
            {
                "http://x.com/": FakeResp('<script src="/app.js"></script><a href="/page">P</a>', 200),
                "http://x.com/app.js": FakeResp("var x=1;", 200, {"Content-Type": "application/javascript"}),
                "http://x.com/page": FakeResp("page", 200),
            }
        )
        c = Crawler(max_depth=2, max_pages=10)
        ...
        sess = FakeSession(
            {
                "https://crt.sh/?q=%25.example.com&output=json": FakeResp(
                    json.dumps(mock_crtsh_response), 200, {"Content-Type": "application/json"}
                ),
            }
        )
        enum = SubdomainEnumerator(
...
class TestJSExtractor:
    ...
    def test_extract_from_urls_with_session(self):
        ...
        ext = JSExtractor()
        endpoints = ext.extract_from_urls(["http://x.com/a.js", "http://x.com/b.js"], session=sess)
        urls = [ep.url for ep in endpoints]
    ...
    def test_extract_from_urls_handles_404(self):
        ...
        sess = FakeSession(
            {
                "http://x.com/ok.js": FakeResp('var a="/api/x";', 200, {"Content-Type": "application/javascript"}),
            }
        )
        # missing.js 不在 responses → 404
        ext = JSExtractor()
        endpoints = ext.extract_from_urls(["http://x.com/ok.js", "http://x.com/missing.js"], session=sess)
        urls = [ep.url for ep in endpoints]
    ...
    def test_endpoint_dataclass(self):
        """Endpoint 数据类"""
        ep = Endpoint(url="/api/x", source="http://x.com/app.js", method="GET", line_no=10, is_absolute=False)
        assert ep.url == "/api/x"
...
class TestOrchestratorD14Integration:
    ...
    def test_run_recon_subdomain_only(self):
        ...
        with patch("lib.subdomain.SubdomainEnumerator.enumerate", return_value=["example.com", "www.example.com"]):
            result = orch._run_recon(req, "http://example.com/", lambda *a: None, "test-task")
    ...
    def test_run_recon_crawl_only(self):
        ...
        with patch("lib.crawler.Crawler.crawl_with_js_urls", return_value=mock_result):
            result = orch._run_recon(req, "http://x.com/", lambda *a: None, "test-task")
    ...
    def test_run_recon_crawl_and_js_extract(self):
        ...
        mock_endpoints = [
            ...
            Endpoint(url="https://x.com/abs", source="http://x.com/app.js", is_absolute=True),
        ]
        with patch("lib.crawler.Crawler.crawl_with_js_urls", return_value=mock_crawl), patch(
            "lib.js_extractor.JSExtractor.extract_from_urls", return_value=mock_endpoints
        ):
            result = orch._run_recon(req, "http://x.com/", lambda *a: None, "test-task")
