...
def test_scan_task_to_dict():
    ...
    task.results = [
        ScanResult(
            kind="vuln", name="SQL注入", severity="high", status=STATUS_CONFIRMED, url="http://x.com/", evidence="命中"
        ),
    ]
    task.duration = 12.5
...
def test_submit_returns_task_id():
    ...
    try:
        ...
        with patch("core.orchestrator.detect_cms") as mock_cms, patch(
            "core.orchestrator.detect_waf"
        ) as mock_waf, patch("core.orchestrator.load_plugins") as mock_load:
            mock_cms.return_value = MagicMock(cms="", version="", confidence=0, matched=[])
            mock_waf.return_value = {"waf": "", "display": "", "bypass_hint": ""}
            mock_load.return_value = []
