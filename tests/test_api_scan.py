...
def test_get_scan_status(client, mock_network):
    ...
    wait_for(
        lambda: client.get(f"/api/scan/{task_id}").json().get("status") in ("done", "failed", "running"), timeout=3
    )
...
def test_fingerprint_probe(client):
    ...
    with patch("api.routes.system.detect_cms") as mock_cms, patch("api.routes.system.detect_waf") as mock_waf:
