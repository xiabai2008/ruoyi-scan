...
class TestECSFormat:
    ...
    def test_render_ecs(self):
        ...
        results = [
            ScanResult(
                kind="vuln", name="XSS", severity=SEVERITY_MEDIUM, status=STATUS_CONFIRMED, url="http://x.com/xss"
            ),
        ]
    ...
    def test_ecs_non_confirmed(self):
        ...
        r = ScanResult(kind="vuln", name="XSS", severity=SEVERITY_LOW, status=STATUS_SAFE, url="http://x.com/xss")
...
class TestCEFFormat:
    ...
    def test_render_cef(self):
        ...
        results = [
            ...
            ScanResult(
                kind="vuln", name="SQLi", severity=SEVERITY_HIGH, status=STATUS_CONFIRMED, url="http://x.com/sqli"
            ),
        ]
...
class TestJSONFormat:
    ...
    def test_render_json(self):
        ...
        results = [
            ScanResult(kind="vuln", name="A", severity=SEVERITY_LOW, status=STATUS_CONFIRMED, url="http://x.com/a"),
        ]
...
class TestSIEMUnified:
    ...
    def test_render_siem_all_formats(self):
        ...
        results = [
            ScanResult(kind="vuln", name="Test", severity=SEVERITY_HIGH, status=STATUS_CONFIRMED, url="http://x.com"),
        ]
    ...
    def test_export_to_files(self, tmp_path):
        ...
        results = [
            ScanResult(kind="vuln", name="A", severity=SEVERITY_HIGH, status=STATUS_CONFIRMED, url="http://x.com/a"),
        ]
        paths = export_to_files(results, ["ecs", "cef", "json"], str(tmp_path), target="http://x.com")
        assert len(paths) == 3
        for p in paths:
...
class TestSyslogForward:
    ...
    def test_send_results_to_syslog(self):
        ...
        results = [
            ScanResult(kind="vuln", name="A", severity=SEVERITY_HIGH, status=STATUS_CONFIRMED, url="http://x.com"),
        ]
        # 使用不存在的端口，不应抛异常
