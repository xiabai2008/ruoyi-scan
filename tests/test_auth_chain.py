...
class TestLoginV4Session(unittest.TestCase):
    ...
    def test_login_success_code_0(self, m):
        ...
        m.get(MOCK_TARGET + '/login', text='<html><form>登录</form></html>',
    ...
    def test_login_success_code_200(self, m):
        ...
        m.get(MOCK_TARGET + '/login', text='<html><form>登录</form></html>',
              headers={'Content-Type': 'text/html'})
        m.post(MOCK_TARGET + '/login', text='{"code":200,"msg":"操作成功"}',
               headers={'Content-Type': 'application/json'})
        chain = RuoYiAuthChain(MOCK_TARGET, SessionManager())
        ok, reason = chain.login()
        self.assertTrue(ok)
    ...
    def test_login_wrong_password(self, m):
        ...
        m.get(MOCK_TARGET + '/login', text='<html><form>登录</form></html>',
              headers={'Content-Type': 'text/html'})
        m.post(MOCK_TARGET + '/login', text='{"code":500,"msg":"用户或密码错误"}',
               headers={'Content-Type': 'application/json'})
        chain = RuoYiAuthChain(MOCK_TARGET, SessionManager())
        ok, reason = chain.login()
        self.assertFalse(ok)
...
class TestLoginV5Jwt(unittest.TestCase):
    ...
    def test_login_success(self, m):
        ...
        m.get(MOCK_TARGET + '/login', text='{"code":401,"msg":"未登录"}',
              headers={'Content-Type': 'application/json'})
        ...
        self.assertEqual(sess.session.headers.get('Authorization'),
    ...
    def test_login_captcha(self, m):
        ...
        m.get(MOCK_TARGET + '/login', text='{"code":401,"msg":"未登录"}',
              headers={'Content-Type': 'application/json'})
        m.post(MOCK_TARGET + '/login', text='{"code":500,"msg":"captcha error"}',
               headers={'Content-Type': 'application/json'})
        chain = RuoYiAuthChain(MOCK_TARGET, SessionManager())
        ok, reason = chain.login()
        self.assertFalse(ok)
