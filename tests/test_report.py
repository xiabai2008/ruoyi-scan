...
from common.models import SEVERITY_HIGH, SEVERITY_MEDIUM, STATUS_CONFIRMED, STATUS_SAFE, STATUS_UNKNOWN, ScanResult
...
def _sample_results():
    ...
    return [
        ...
        ScanResult(kind="vuln", name="不存在漏洞X", severity="low", status=STATUS_SAFE, url="http://target/safe"),
        ScanResult(
            kind="vuln",
            name="网络异常项Y",
...
def test_donut_svg_content():
    ...
    assert "确认漏洞" in svg
    print(
        "PASS test_donut_svg_content: total=%d high=%d medium=%d low=%d"
...
def test_batch_bar_svg():
    ...
    from common.models import SEVERITY_HIGH, ScanResult
    from core.report import BatchReport

    b1 = ReportBuilder(results=_sample_results(), target="http://t1/", summary=_sample_summary())
