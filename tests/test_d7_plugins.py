...
def test_all_bypass_plugins_support_waf_bypass():
    ...
    for plugin_cls, _ in _BYPASS_PLUGINS:
        assert plugin_cls.supports_waf_bypass is True, f"{plugin_cls.__name__}.supports_waf_bypass 应为 True"


def test_no_bypass_plugins_not_supported():
    """不支持绕过的插件 supports_waf_bypass=False"""
    for plugin_cls in _NO_BYPASS_PLUGINS:
        assert plugin_cls.supports_waf_bypass is False, f"{plugin_cls.__name__}.supports_waf_bypass 应为 False"
...
def test_all_plugins_have_bypass_max_attempts():
    ...
    for plugin_cls, _ in all_plugins:
        assert hasattr(plugin_cls, "bypass_max_attempts"), f"{plugin_cls.__name__} 缺 bypass_max_attempts"
...
def test_all_plugins_have_verify_with_bypass():
    ...
    all_plugins = _BYPASS_PLUGINS + [(cls, "") for cls in _NO_BYPASS_PLUGINS]
    for plugin_cls, _ in all_plugins:
        assert hasattr(plugin_cls, "verify_with_bypass"), f"{plugin_cls.__name__} 应继承 verify_with_bypass"
...
def test_vuln_type_coverage():
    ...
    assert vuln_types == expected_types, f"漏洞类型不完整: {vuln_types} vs {expected_types}"
...
def test_waf_features_have_block_signatures():
    ...
    for waf_name, waf_data in WAF_FEATURES.items():
        ...
        assert len(waf_data["block_signatures"]) > 0, f"{waf_name} 的 block_signatures 不应为空"
