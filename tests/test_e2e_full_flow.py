...
class TestE2EFullFlow(unittest.TestCase):
    ...
    def test_core_imports_with_type_hints(self):
        ...
        result = ScanResult(
            kind="vuln", name="test", severity="high", status=STATUS_CONFIRMED, url="http://test/", evidence="test"
        )
    ...
    def test_report_builder_basic(self):
        ...
        builder = ReportBuilder(results=results, target="http://test/", summary={"mode": "test", "duration": 1.0})
    ...
    def test_scan_result_three_states(self):
        ...
        confirmed = ScanResult(
            kind="vuln", name="test", severity="high", status=STATUS_CONFIRMED, url="http://t/", evidence="e"
        )
        safe = ScanResult(kind="vuln", name="test", severity="low", status=STATUS_SAFE, url="http://t/", evidence="e")
        unknown = ScanResult(
            kind="vuln", name="test", severity="medium", status=STATUS_UNKNOWN, url="http://t/", evidence="e"
        )
