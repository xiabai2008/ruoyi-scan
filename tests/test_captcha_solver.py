...
class TestDetectCaptcha(unittest.TestCase):
    ...
    def test_detect_image_captcha(self, m):
        ...
        m.get(MOCK_TARGET + "/captcha/captchaImage", content=TEST_PNG_BYTES, headers={"Content-Type": "image/png"})
...
class TestSolve(unittest.TestCase):
    ...
    def test_solve_empty_image(self, m):
        ...
        m.get(MOCK_TARGET + "/captcha/captchaImage", content=b"", headers={"Content-Type": "image/png"})
        solver = CaptchaSolver(MOCK_TARGET, SessionManager())
        has, code = solver.solve()
        self.assertTrue(has)
        self.assertEqual(code, "")
