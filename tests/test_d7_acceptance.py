...
class WafBlockedSqliPlugin(PluginBase):
    ...
    def verify(self, target, session):
        ...
        resp = session.post(
            url, data={"params[dataScope]": "and extractvalue(1,concat(0x7e,(select database()),0x7e))"}
        )
        if resp.status_code == 403 or "blocked" in resp.text.lower():
            return ScanResult(kind="vuln", name=self.name, status=STATUS_SAFE, url=url, evidence="WAF 拦截，未确认漏洞")
        # 无 WAF 时正常检测
    ...
    def verify_with_bypass(self, target, bypass_session, bypass_ctx):
        ...
        return ScanResult(kind="vuln", name=self.name, status=STATUS_SAFE, url=url, evidence="绕过后仍未命中")
...
class WafBlockedFileReadPlugin(PluginBase):
    ...
    def verify(self, target, session):
        ...
        if resp.status_code == 403:
            return ScanResult(kind="vuln", name=self.name, status=STATUS_SAFE, url=url, evidence="WAF 拦截")
...
class WafMockSession:
    ...
    def _respond(self, req):
        ...
        is_tampered = any(
            sig in payload
            for sig in ["/**/", "BETWEEN", "%61nd", "%65xtractvalue", "/*!", "aNd", "eXtractvalue", "cOn cat"]
        )

        # 原始 payload + 无绕过 → WAF 拦截
        if has_blocked_kw and not (is_googlebot or is_chunked or is_tampered):
            return MagicMock(text="Attention Required! | cloudflare", status_code=403, headers={"CF-Ray": "abc123"})
        ...
        if (has_blocked_kw or is_tampered) and (is_googlebot or is_chunked or is_tampered):
            return MagicMock(text="运行时异常 database() error", status_code=200, headers={})

        if "/etc/passwd" in url and (is_googlebot or is_chunked):
            return MagicMock(text="root:x:0:0:root:/root:/bin/bash", status_code=200, headers={})
...
def test_e2e_engine_full_flow():
    ...
    results = engine.run([WafBlockedSqliPlugin], "http://target.com/", session, waf_bypass_coordinator=coord)
...
def test_protection_matrix_confirmed_not_bypassed():
    ...
    confirmed = ScanResult(
        kind="vuln", name=plugin.name, status=STATUS_CONFIRMED, url="http://x.com/", evidence="已确认"
    )
...
def test_protection_matrix_no_vuln_type_not_bypassed():
    ...
    class NoVulnTypePlugin(PluginBase):
        ...
        # vuln_type 为空
        def verify(self, target, session):
            return ScanResult(kind="vuln", name=self.name, status=STATUS_SAFE, url=target, evidence="safe")
...
def test_protection_matrix_bypass_failure_preserves_safe():
    ...
    class AlwaysFailBypassPlugin(PluginBase):
        ...
        def verify_with_bypass(self, target, bypass_session, bypass_ctx):
            return ScanResult(kind="vuln", name=self.name, status=STATUS_SAFE, url=target, evidence="bypass also safe")
...
def test_bypass_session_injects_transport_or_tamper():
    ...
    assert has_tampered or has_transport, "绕过请求应含 payload 变形或传输层变换"
...
def test_multiple_strategies_tried_until_success():
    ...
    class MultiStrategyPlugin(PluginBase):
        ...
        def verify_with_bypass(self, target, bypass_session, bypass_ctx):
            ...
            return ScanResult(
                kind="vuln", name=self.name, status=STATUS_SAFE, url=target, evidence=f"第{call_count[0]}次失败"
            )
...
def test_bypass_max_attempts_respected():
    ...
    class LimitedAttemptsPlugin(PluginBase):
        ...
        def verify_with_bypass(self, target, bypass_session, bypass_ctx):
            call_count[0] += 1
            return ScanResult(kind="vuln", name=self.name, status=STATUS_SAFE, url=target, evidence="always safe")
...
def test_engine_bypass_error_marked_in_extra():
    ...
    class ErrorPlugin(PluginBase):
        ...
        def verify(self, target, session):
            return ScanResult(kind="vuln", name=self.name, status=STATUS_UNKNOWN, url=target, evidence="unknown")
    ...
    session = WafMockSession()
    results = engine.run([ErrorPlugin], "http://x.com/", session, waf_bypass_coordinator=ErrorCoordinator())
    assert len(results) == 1
