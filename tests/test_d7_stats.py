...
class SuccessOnSecondStrategyPlugin(PluginBase):
    ...
    def verify_with_bypass(self, target, bypass_session, bypass_ctx):
        ...
        if self._call_count >= 2:
            return ScanResult(
                kind="vuln", name=self.name, severity="high", status=STATUS_CONFIRMED, url=target, evidence="第二次成功"
            )
        return ScanResult(kind="vuln", name=self.name, status=STATUS_SAFE, url=target, evidence="失败")
...
def test_coordinator_without_stats_tracker_works():
    ...
    class SimplePlugin(PluginBase):
        ...
        def verify_with_bypass(self, target, bypass_session, bypass_ctx):
            return ScanResult(
                kind="vuln", name=self.name, severity="high", status=STATUS_CONFIRMED, url=target, evidence="成功"
            )
