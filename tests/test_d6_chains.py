...
class MockSession:
    ...
    def _match_response(self, url):
        ...
        for substring, config in self._responses.items():
            if substring in url:
                return MockResponse(text=config['text'],
...
def test_chain1_full_success():
    ...
    session = MockSession()
    # 步骤 1：SQL 注入返回数据库名
    session.set_response('/system/role/list',
                          text='{"msg":"XPATH syntax error: \'~ry\'"}')
    # 步骤 2：配置文件读取返回含密码的配置
    session.set_response('/common/download/resource',
                          text='spring:\n  datasource:\n    password: root123\n    url: jdbc:mysql://localhost:3306/ry')
    # 步骤 3：定时任务接口返回 jobs 列表
    session.set_response('/monitor/job/list',
                          text='{"rows":[{"jobId":1,"invokeTarget":"ruoYi.run"}]}')
...
def test_chain3_full_success():
    ...
    session.set_response('/v1/auth/users',
                          text='{"totalCount":1,"pageNumber":1,"pageSize":1,"pageItems":[{"username":"nacos"}]}')
    # 步骤 1：Nacos 配置列表可读（含 pageItems）
    session.set_response('/v1/cs/configs',
                          text='{"totalCount":1,"pageItems":[{"dataId":"application.yml","group":"DEFAULT_GROUP"}]}')
    ...
    assert result.status in [CHAIN_CONFIRMED, CHAIN_PARTIAL], \
        f'应 CONFIRMED 或 PARTIAL，实际 {result.status}'
