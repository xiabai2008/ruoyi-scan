...
class BypassSupportedPlugin(PluginBase):
    ...
    def verify(self, target, session):
        return ScanResult(kind="vuln", name=self.name, status=STATUS_SAFE, url=target, evidence="safe (maybe blocked)")
...
def test_engine_bypass_triggered_for_supported_plugin():
    ...
    session = MockSession()
    results = engine.run([BypassSupportedPlugin], "http://x.com/", session, waf_bypass_coordinator=coord)
...
def test_engine_bypass_not_triggered_for_unsupported_plugin():
    ...
    coord = MockBypassCoordinator(should_succeed=True)
    session = MockSession()
    results = engine.run([BypassUnsupportedPlugin], "http://x.com/", session, waf_bypass_coordinator=coord)
...
def test_engine_bypass_not_triggered_for_confirmed():
    ...
    class ConfirmedPlugin(PluginBase):
        ...
        def verify(self, target, session):
            return ScanResult(kind="vuln", name=self.name, status=STATUS_CONFIRMED, url=target, evidence="confirmed")

    engine = ScanEngine(threads=1)
    coord = MockBypassCoordinator(should_succeed=True)
    session = MockSession()
    results = engine.run([ConfirmedPlugin], "http://x.com/", session, waf_bypass_coordinator=coord)
