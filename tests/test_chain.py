...
class MockSafePlugin(PluginBase):
    ...
    def verify(self, target, session):
        return ScanResult(kind="vuln", name=self.name, status=STATUS_SAFE, url=target, evidence="mock safe")
...
class MockUnknownPlugin(PluginBase):
    ...
    def verify(self, target, session):
        return ScanResult(kind="vuln", name=self.name, status=STATUS_UNKNOWN, url=target, evidence="mock unknown")
...
def test_topological_sort_parallel():
    ...
    chain = _make_chain(
        [
            ...
            ChainStep(id="merge", plugin_cls=MockConfirmedPlugin, depends_on=["branch_a", "branch_b"]),
        ]
    )
    engine = ChainEngine()
    order = engine._topological_sort(chain)
...
def test_condition_skipped():
    ...
    chain = _make_chain(
        [
            ChainStep(id="a", plugin_cls=MockConfirmedPlugin),
            ChainStep(id="b", plugin_cls=MockConfirmedPlugin, condition=lambda ctx: False),  # 永远 False
        ]
    )
...
def test_condition_eval_exception():
    ...
    chain = _make_chain(
        [
            ChainStep(id="a", plugin_cls=MockConfirmedPlugin, condition=bad_condition),
        ]
    )
    result = _run_chain(chain)
...
def test_failure_fallback():
    ...
    chain = _make_chain(
        [
            ChainStep(id="main", plugin_cls=MockSafePlugin, on_fail=ON_FAIL_FALLBACK, fallback_steps=["backup"]),
...
def test_context_fact_extraction():
    ...
    chain = _make_chain(
        [
            ChainStep(
                id="a", plugin_cls=MockExtractPlugin, outputs={"db_name": "extra:db_name", "db_url": "field:url"}
            ),
        ]
    )
    result = _run_chain(chain)
...
def test_context_secret_masking():
    ...
    chain = _make_chain(
        [
            ChainStep(id="a", plugin_cls=MockExtractPlugin, outputs={"db_password": "secret:db_password"}),
        ]
    )
    engine = ChainEngine()
