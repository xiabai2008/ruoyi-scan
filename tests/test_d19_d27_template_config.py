...
from lib.config_loader import (
    set_parser_defaults as cl_set_defaults,
)
...
class TestFilterPlugins:
    ...
    def test_filter_by_severity(self):
        ...
        tmpl = ScanTemplate(name="t", display_name="T", description="d", severity_filter={"high"})
    ...
    def test_filter_by_category(self):
        ...
        tmpl = ScanTemplate(name="t", display_name="T", description="d", category_filter={"vuln"})
        filtered = filter_plugins(plugins, tmpl)
        assert len(filtered) == 1
    ...
    def test_filter_by_compliance_owasp(self):
        ...
        tmpl = ScanTemplate(name="t", display_name="T", description="d", compliance_filter={"OWASP"})
        filtered = filter_plugins(plugins, tmpl)
        assert len(filtered) == 1
    ...
    def test_filter_by_compliance_dengbao(self):
        ...
        tmpl = ScanTemplate(name="t", display_name="T", description="d", compliance_filter={"等保"})
        filtered = filter_plugins(plugins, tmpl)
        assert len(filtered) == 1
    ...
    def test_filter_real_plugins_compliance(self):
        ...
        for cls in filtered:
            assert "OWASP" in (getattr(cls, "compliance", "") or ""), f"{cls.__name__} 应含 OWASP 映射"
