...
def test_metrics_storage_count_increases(client, app):
    ...
    assert new_count >= initial_count + 1, \
        f'storage_tasks 应至少 +1，初始 {initial_count}，新值 {new_count}'
...
def test_metrics_accessible_with_api_key(client):
    ...
    try:
        ...
        with TestClient(app_with_key) as c:
            ...
            assert resp.status_code == 200, \
                f'metrics 在 PUBLIC_PATHS 中，应免鉴权，实际 {resp.status_code}'
    finally:
        import shutil
        shutil.rmtree(tmpdir, ignore_errors=True)
...
if __name__ == '__main__':
    ...
    try:
        ...
        with TestClient(app) as c:
            # 注册任务
            app.state.registry.register('manual-001', {
                'task_id': 'manual-001', 'status': 'pending', 'target': 'http://x/',
            })
            resp = c.get('/api/system/metrics')
