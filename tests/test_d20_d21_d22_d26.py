    ...
    def _make_builder(self):
        ...
        results = [
            ScanResult(
                ...
                url="http://x/system/role",
                evidence="database() 报错",
                fix="参数化查询",
                cve="CVE-2023-1234",
                cvss_score=9.8,
            ...
            ScanResult(
                ...
                status=STATUS_CONFIRMED,
                url="http://x/common/download",
                evidence="/etc/passwd",
                fix="限制路径",
            ),
            ScanResult(kind="vuln", name="安全项", severity=SEVERITY_LOW, status=STATUS_SAFE, url="http://x/safe"),
        ]
        return ReportBuilder(results=results, target="http://x/", summary={"started_at": "2026-01-01", "duration": 1.0})
...
class TestBuildNotificationMessage:
    ...
    def _make_builder(self):
        results = [
            ...
            ScanResult(
                kind="vuln", name="文件读取", severity=SEVERITY_MEDIUM, status=STATUS_CONFIRMED, url="http://x/b"
            ),
        ]
        return ReportBuilder(
            results=results,
            target="http://x/",
            summary={"started_at": "2026-01-01", "duration": 5.0, "request_count": 10, "mode": "综合"},
        )
    ...
    def test_build_message_truncation(self):
        ...
        results = [
            ScanResult(
                kind="vuln", name=f"vuln-{i}", severity=SEVERITY_HIGH, status=STATUS_CONFIRMED, url=f"http://x/{i}"
            )
            for i in range(25)
        ]
...
class TestD20D22Integration:
    ...
    def test_diff_with_real_report(self):
        ...
        old_builder = ReportBuilder(
            results=[
                ScanResult(kind="vuln", name="A", severity=SEVERITY_HIGH, status=STATUS_CONFIRMED, url="http://x/a")
            ],
            target="http://x/",
            summary={"started_at": "2026-01-01"},
        )
        # 新报告：A 已修复，新增 B
        new_builder = ReportBuilder(
            results=[
                ScanResult(kind="vuln", name="B", severity=SEVERITY_HIGH, status=STATUS_CONFIRMED, url="http://x/b")
            ],
            target="http://x/",
            summary={"started_at": "2026-01-02"},
        )
