...
def test_fingerprint_normalizes_url():
    ...
    r1 = ScanResult(kind='vuln', name='任意文件读取', url='http://x.com/a?b=1',
                    extra={'vuln_type': 'arbitrary_file_read', 'payload_class': 'p1'})
    r2 = ScanResult(kind='vuln', name='任意文件读取', url='http://x.com/a?b=2',
                    extra={'vuln_type': 'arbitrary_file_read', 'payload_class': 'p1'})
...
def test_fingerprint_different_endpoint():
    ...
    r1 = ScanResult(kind='vuln', name='SQL注入', url='http://x.com/system/role/list',
                    extra={'vuln_type': 'sql_injection_error_based'})
    r2 = ScanResult(kind='vuln', name='SQL注入', url='http://x.com/system/dept/list',
                    extra={'vuln_type': 'sql_injection_error_based'})
    assert fingerprint(r1) != fingerprint(r2)
...
def test_fingerprint_vuln_type_fallback():
    ...
    r2 = ScanResult(kind='vuln', name='任意文件读取（路径穿越）', url='http://x.com/a',
                    extra={'vuln_type': 'arbitrary_file_read'})
...
def test_aggregate_no_merge():
    """4 条不同漏洞,无合并"""
    results = [
        ScanResult(kind='vuln', name='漏洞A', severity=SEVERITY_HIGH,
                   status=STATUS_CONFIRMED, url='http://x.com/a',
                   extra={'vuln_type': 'rce'}),
        ScanResult(kind='vuln', name='漏洞B', severity=SEVERITY_MEDIUM,
                   status=STATUS_CONFIRMED, url='http://x.com/b',
                   extra={'vuln_type': 'ssrf'}),
        ScanResult(kind='vuln', name='漏洞C', severity=SEVERITY_LOW,
                   status=STATUS_SAFE, url='http://x.com/c',
                   extra={'vuln_type': 'xss'}),
        ScanResult(kind='vuln', name='漏洞D', severity=SEVERITY_HIGH,
                   status=STATUS_UNKNOWN, url='http://x.com/d',
                   extra={'vuln_type': 'xxe'}),
    ]
...
def test_aggregate_merge_file_read_pair():
    ...
    results = [
        ScanResult(kind='vuln', name='任意文件读取', severity=SEVERITY_HIGH,
                   ...
                   evidence='响应含 root 与 :/ 特征', fix='限制 resource 参数',
                   extra=extra),
        ScanResult(kind='vuln', name='任意文件读取（路径穿越）', severity=SEVERITY_HIGH,
                   status=STATUS_CONFIRMED,
                   url='http://x.com/common/download/resource?resource=../../../etc/passwd',
                   evidence='读取到 /etc/passwd', fix='resource 参数白名单校验',
                   extra=extra),
    ]
    aggregated, report = aggregate(results)
...
def test_aggregate_keeps_highest_severity():
    ...
    results = [
        ScanResult(kind='vuln', name='RCE', severity=SEVERITY_MEDIUM,
                   status=STATUS_CONFIRMED, url='http://x.com/a', extra=extra),
        ScanResult(kind='vuln', name='RCE', severity=SEVERITY_HIGH,
                   status=STATUS_CONFIRMED, url='http://x.com/a?b=1', extra=extra),
    ]
...
def test_aggregate_keeps_worst_status():
    ...
    results = [
        ScanResult(kind='vuln', name='RCE', severity=SEVERITY_HIGH,
                   status=STATUS_UNKNOWN, url='http://x.com/a', extra=extra),
        ScanResult(kind='vuln', name='RCE', severity=SEVERITY_HIGH,
                   status=STATUS_CONFIRMED, url='http://x.com/a?b=1', extra=extra),
    ]
    aggregated, _ = aggregate(results)
...
def test_aggregate_evidence_joined():
    ...
    results = [
        ScanResult(kind='vuln', name='RCE', severity=SEVERITY_HIGH,
                   status=STATUS_CONFIRMED, url='http://x.com/a',
                   evidence='证据A', extra=extra),
        ScanResult(kind='vuln', name='RCE', severity=SEVERITY_HIGH,
                   status=STATUS_CONFIRMED, url='http://x.com/a?b=1',
                   evidence='证据B', extra=extra),
    ]
    aggregated, _ = aggregate(results)
...
def test_aggregate_fix_longest():
    ...
    results = [
        ScanResult(kind='vuln', name='RCE', severity=SEVERITY_HIGH,
                   status=STATUS_CONFIRMED, url='http://x.com/a',
                   fix=short_fix, extra=extra),
        ScanResult(kind='vuln', name='RCE', severity=SEVERITY_HIGH,
                   status=STATUS_CONFIRMED, url='http://x.com/a?b=1',
                   fix=long_fix, extra=extra),
    ]
    aggregated, _ = aggregate(results)
...
def test_aggregate_preserves_order():
    ...
    results = [
        ScanResult(kind='vuln', name='Z漏洞', severity=SEVERITY_LOW,
                   status=STATUS_SAFE, url='http://x.com/c'),
        ScanResult(kind='vuln', name='A漏洞', severity=SEVERITY_HIGH,
                   status=STATUS_CONFIRMED, url='http://x.com/a'),
        ScanResult(kind='vuln', name='B漏洞', severity=SEVERITY_MEDIUM,
                   status=STATUS_CONFIRMED, url='http://x.com/b'),
    ]
    aggregated, _ = aggregate(results)
...
def test_aggregated_vuln_duck_types_scanresult():
    ...
    av = AggregatedVuln(
        name='测试漏洞', severity=SEVERITY_HIGH, status=STATUS_CONFIRMED,
        url='http://x.com/a', evidence='证据', fix='修复', kind='vuln',
        extra={'vuln_type': 'rce'}, fingerprint='abc123', hit_count=2,
    )
...
def test_dedup_report_stats():
    ...
    results = [
        ...
        ScanResult(kind='vuln', name='SQL注入', url='http://x.com/sqli',
                   extra={'vuln_type': 'sql_injection_error_based'}),
        ScanResult(kind='vuln', name='RCE', url='http://x.com/rce',
                   extra={'vuln_type': 'rce'}),
    ]
    aggregated, report = aggregate(results)
...
def test_aggregate_single_result():
    """1 条结果不报错"""
    results = [
        ScanResult(kind='vuln', name='单条漏洞', severity=SEVERITY_HIGH,
                   status=STATUS_CONFIRMED, url='http://x.com/a',
                   extra={'vuln_type': 'rce'}),
    ]
    aggregated, report = aggregate(results)
