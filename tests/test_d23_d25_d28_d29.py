...
class TestD23D25D28D29Integration:
    ...
    def test_ci_with_i18n(self):
        ...
        # CI 摘要始终用英文（适合国际日志）
        results = [
            ScanResult(kind="vuln", name="Test", severity=SEVERITY_HIGH, status=STATUS_CONFIRMED, url="http://x/")
        ]
    ...
    def test_wiki_with_ci_exit_code(self):
        ...
        # CI 退出码独立计算
        results = [
            ScanResult(kind="vuln", name="High", severity=SEVERITY_HIGH, status=STATUS_CONFIRMED, url="http://x/")
        ]
        assert get_ci_exit_code(results, "high") == 1
