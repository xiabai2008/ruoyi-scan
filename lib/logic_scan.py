...
# 常见对象引用参数名
ID_PARAM_PATTERNS = ['id', 'uid', 'userid', 'orderid', 'docid', 'fileid',
                     'recordid', 'itemid', 'accountid', 'deptid', 'roleid',
                     'menuid', 'configid', 'postid', 'noticeid']
    ...
    def detect_id_params(self, url: str, params: List[str]) -> List[str]:
        ...
        for p in params:
            if p.lower() in ID_PARAM_PATTERNS:
                id_params.append(p)
            elif re.search(r'(id|Id|ID)$', p):
                id_params.append(p)
        return id_params

    def test_idor(self, endpoint: EndpointInfo,
                  test_ids: List[str] = None) -> Optional[LogicVuln]:
        """测试单个端点的 IDOR
        ...
        for test_id in test_ids:
            ...
            if resp_code == 200 and resp_size > 100:
                # 检查是否含权限拒绝关键字
                denied_keywords = ['无权限', '权限不足', 'forbidden', 'unauthorized',
                                   '禁止访问', '没有权限', 'access denied']
...
# 管理端点常见路径模式
ADMIN_PATH_PATTERNS = [
    r'/admin/', r'/manage/', r'/system/', r'/sys/',
    r'/backend/', r'/console/', r'/dashboard/',
    r'/api/admin/', r'/api/system/', r'/api/manage/',
]
...
class PrivilegeEscalationDetector:
    ...
    def detect_admin_endpoints(self, base_url: str,
        ...
        if paths is None:
            paths = [
                '/admin/index', '/admin/list', '/admin/user',
                '/system/user/list', '/system/role/list', '/system/menu/list',
                '/system/config/list', '/system/dept/list',
                '/manage/dashboard', '/console/api/users',
            ]
...
class ParameterTamperingDetector:
    ...
    def test_parameter_tampering(self, endpoint: EndpointInfo,
                                 param: str,
                                 original_value: str = '100',
                                 tampered_value: str = '0.01') -> Optional[LogicVuln]:
        """测试参数篡改
        ...
        if resp_code == 200:
            ...
            if any(kw in resp_text.lower() for kw in success_keywords):
                return LogicVuln(
                    ...
                    url=test_url,
                    method='GET',
                    evidence=f'篡改 {param}={tampered_value}（原值 {original_value}）'
                            f'服务端返回 200 成功',
...
class RaceConditionDetector:
    ...
    def test_race_condition(self, endpoint: EndpointInfo,
                            expected_success_count: int = 1) -> Optional[LogicVuln]:
        """测试竞争条件
