...
class AsyncScanEngine:
    ...
    async def submit_async(self, fn: Callable, *args, **kwargs) -> Any:
        ...
        return await loop.run_in_executor(
            self._executor,
            lambda: self._wrap_task(fn, *args, **kwargs)
        )
...
def scan_batch_targets(scan_fn: Callable[[str], List[Any]],
                       targets: List[str],
                       max_workers: int = 10,
...
def scan_plugins_concurrent(plugin_verify_fn: Callable,
                            plugins: List[Any],
                            target: str,
                            session,
...
async def async_http_get(url: str, headers: Dict = None,
                         timeout: float = 10.0, verify_ssl: bool = False) -> Dict[str, Any]:
...
async def async_http_post(url: str, data: Dict = None, json_data: Dict = None,
                          headers: Dict = None, timeout: float = 10.0) -> Dict[str, Any]:
    """异步 HTTP POST 请求"""
    try:
        import aiohttp
    except ImportError:
...
def benchmark_sync_vs_async(sync_fn: Callable, targets: List[str],
...
def run_async_scan_mode(scan_fn: Callable[[str], List[Any]],
                        targets: List[str],
                        max_workers: int = 10,
                        progress_callback: Optional[Callable] = None) -> List[Any]:
    """异步扫描模式入口
