...
def to_ecs_event(result, target: str = '', scan_time: str = '') -> Dict[str, Any]:
    ...
    # 合规标签
    if compliance:
        event['vulnerability']['compliance'] = [
            f'{k}:{v}' for k, v in compliance.items()
        ]
...
def render_siem(results, format: str, target: str = '',
                scan_time: str = '') -> str:
    """渲染扫描结果为指定 SIEM 格式
...
def export_to_files(results, formats: List[str], output_dir: str,
                    target: str = '', scan_time: str = '') -> List[str]:
    """导出多格式到文件
...
def send_to_syslog(events: List[str], host: str, port: int = 514,
                   protocol: str = 'udp', timeout: float = 5.0) -> int:
    """发送事件到 Syslog 服务器
...
def send_results_to_syslog(results, host: str, port: int = 514,
                           format: str = 'cef', protocol: str = 'udp',
                           target: str = '') -> int:
    """发送扫描结果到 Syslog 服务器
...
def run_siem_export_mode(args, results, target: str = '',
                         scan_time: str = '') -> int:
    """SIEM 导出模式入口
