...
def generate_cache_key(target: str, plugin_config: Dict = None, scan_mode: str = "") -> str:
    """生成缓存键
...
def generate_plugin_cache_key(target: str, plugin_name: str, plugin_config: Dict = None) -> str:
    """生成单个插件的缓存键
...
class CacheStorage:
    ...
    def set(self, cache_key: str, target: str, result: Dict[str, Any], ttl: int = 3600, plugin_name: str = "") -> None:
        """设置缓存
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            # 过期条目数
            now = datetime.datetime.now().isoformat()
            expired = conn.execute("SELECT COUNT(*) FROM cache_entries WHERE expires_at < ?", (now,)).fetchone()[0]

            # 总命中次数
            total_hits = conn.execute("SELECT COALESCE(SUM(hit_count), 0) FROM cache_entries").fetchone()[0]

            # 高频命中（top 5）
            top = conn.execute("SELECT target, hit_count FROM cache_entries ORDER BY hit_count DESC LIMIT 5").fetchall()
...
class ScanCache:
    ...
    def get_scan_result(self, target: str, scan_mode: str = "", plugin_config: Dict = None) -> Optional[Dict[str, Any]]:
        """查询扫描结果缓存
    ...
    def get_plugin_result(self, target: str, plugin_name: str, plugin_config: Dict = None) -> Optional[Dict[str, Any]]:
        """查询单个插件的缓存结果
    ...
    def set_plugin_result(
        self, target: str, plugin_name: str, result: Dict[str, Any], plugin_config: Dict = None, ttl: int = None
    ) -> str:
        """存储单个插件结果到缓存"""
        cache_key = generate_plugin_cache_key(target, plugin_name, plugin_config)
        cache_data = {
        ...
        self.storage.set(cache_key, target, cache_data, ttl or self.ttl, plugin_name=plugin_name)
        return cache_key
...
def cached_scan(cache: ScanCache, target_arg: str = "target", plugin_name: str = "", ttl: int = None):
    """扫描函数缓存装饰器
