...
class CVEInfo:
    ...
    def __init__(self, cve_id: str, description: str = '',
                 cvss_vector: str = '', cvss_score: float = 0.0,
                 severity: str = '', published: str = '',
                 last_modified: str = '', references: List[str] = None,
                 cwe: List[str] = None):
        self.cve_id = cve_id
...
def query_nvd_api(cve_id: str, timeout: int = 10,
                  api_key: str = None) -> Optional[CVEInfo]:
...
def lookup_cve(cve_id: str, use_cache: bool = True,
               api_key: str = None) -> Optional[CVEInfo]:
    """查询单个 CVE（缓存优先）
...
def batch_lookup_cves(cve_ids: List[str], use_cache: bool = True,
                      api_key: str = None) -> Dict[str, Optional[CVEInfo]]:
    """批量查询 CVE
...
def build_cve_update_report(plugins_cves: List[Tuple[str, str]],
                            cve_infos: Dict[str, Optional[CVEInfo]]) -> Dict[str, Any]:
    """构建 CVE 更新报告
