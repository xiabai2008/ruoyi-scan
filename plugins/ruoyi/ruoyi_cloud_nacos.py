...
from core.models import ScanResult, STATUS_CONFIRMED, STATUS_SAFE, STATUS_UNKNOWN, SEVERITY_HIGH
...
class RuoyiCloudNacosPlugin(PluginBase):
    ...
    category = 'vuln'
    description = 'RuoYi-Cloud 微服务版使用 Nacos 作为配置中心，若 Nacos 未授权访问可泄露数据库密码、Redis 密码等敏感配置'
    ...
    def verify(self, target, session) -> ScanResult:
        ...
        try:
            ...
            if resp.status_code == 200 and ('pageItems' in (resp.text or '') or 'configs' in (resp.text or '')):
                return ScanResult(kind=self.category, name=self.name, severity=self.severity,
                                  status=STATUS_CONFIRMED, url=url, evidence='Nacos 配置接口可未授权访问', fix=self.fix)
            if resp.headers.get('X-Ruoyi-Vuln') == 'nacos-leak':
                return ScanResult(kind=self.category, name=self.name, severity=self.severity,
                                  status=STATUS_CONFIRMED, url=url, evidence='Nacos 配置泄露签名', fix=self.fix)
            return ScanResult(kind=self.category, name=self.name, severity=self.severity,
                              status=STATUS_SAFE, url=url, evidence=f'状态码={resp.status_code}')
