...
class SpringTraceLeakPlugin(PluginBase):
    ...
    def verify(self, target, session):
        ...
        try:
            ...
            emit(no("Spring Actuator /trace 泄露（网络异常）"))
            return ScanResult(kind="vuln", name=self.name, status=STATUS_UNKNOWN, url=url, evidence=str(e))

        text = resp.text or ""
        if TRACE_LEAK_MARKER in text:
            emit(ok("存在 Spring Boot Actuator /trace 请求历史泄露漏洞"))
            return ScanResult(
                kind="vuln",
                name=self.name,
        ...
        if resp.status_code == 200 and match_trace_leak(text):
            emit(ok("存在 Spring Boot Actuator /trace 请求历史泄露漏洞（真实漏洞响应）"))
            return ScanResult(
                kind="vuln",
                name=self.name,
                severity=self.severity,
        ...
        emit(no("不存在 Spring Boot Actuator /trace 请求历史泄露漏洞"))
        return ScanResult(
            kind="vuln",
            name=self.name,
            status=STATUS_SAFE,
