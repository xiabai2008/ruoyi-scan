...
class EnvLeakPlugin(PluginBase):
    ...
    # 正向关键字：典型 .env 文件中的键名
    _ENV_KEYWORDS = ['DB_HOST', 'DB_DATABASE', 'DB_USERNAME', 'DB_PASSWORD',
                     'APP_KEY', 'APP_SECRET', 'REDIS_PASSWORD', 'MAIL_PASSWORD']
    ...
    def verify(self, target, session) -> ScanResult:
        ...
        try:
            ...
            return ScanResult(kind='error', name=self.name, status=STATUS_UNKNOWN,
                              evidence=f'请求异常: {e}')
