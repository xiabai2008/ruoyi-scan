...
class SwaggerLeakPlugin(PluginBase):
    ...
    _SWAGGER_PATHS = [
        ...
        '/doc.html', '/api-docs', '/api.html',
        '/druid/index.html',  # Druid 监控（也属于信息泄露类）
    ]

    _POSITIVE = ['swagger', 'Swagger', 'openapi', 'OpenAPI',
                 'api-docs', 'Knife4j', 'swagger-ui', '"swagger"']
    ...
    def verify(self, target, session) -> ScanResult:
        ...
        for path in self._SWAGGER_PATHS:
            ...
            try:
                ...
                if ('json' in ct and ('"swagger"' in text or '"openapi"' in text or '"paths"' in text)) or \
                   any(kw in text for kw in self._POSITIVE):
                    found.append(path)
            except Exception:
                continue
        if found:
