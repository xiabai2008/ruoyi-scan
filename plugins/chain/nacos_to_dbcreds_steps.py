...
class NacosUnauthPlugin(PluginBase):
    ...
    def verify(self, target, session):
                ...
                try:
                    ...
                    if 'pageItems' in config_resp.text or 'totalCount' in config_resp.text:
                        return ScanResult(kind='chain', name=self.name, severity=self.severity,
                                          ...
                                          fix=self.fix,
                                          extra={'nacos_url': join_url(target, prefix),
                                                 'accessible': True,
        ...
        return ScanResult(kind='chain', name=self.name, status=STATUS_SAFE,
                          url=target, evidence='未检测到 Nacos 服务')
    ...
    def verify(self, target, session):
        ...
        try:
            ...
            return ScanResult(kind='chain', name=self.name, status=STATUS_UNKNOWN,
                              url=list_url, evidence=f'网络异常: {e}')
        ...
        if db_url or db_username:
            ...
            return ScanResult(kind='chain', name=self.name, severity=self.severity,
                              status=STATUS_CONFIRMED, url=list_url,
                              evidence='; '.join(evidence_parts),
                              fix=self.fix,
                              extra={'db_url': db_url, 'db_username': db_username,
                                     'db_password': db_password,
                                     'vuln_type': 'nacos_config_leak'})

        return ScanResult(kind='chain', name=self.name, status=STATUS_SAFE,
                          url=list_url, evidence='未提取到数据库配置')
