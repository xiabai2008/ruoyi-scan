        ...
        for path in config_paths:
            ...
            if db_password or redis_password:
                ...
                return ScanResult(kind='chain', name=self.name, severity=self.severity,
                                  ...
                                  fix=self.fix,
                                  extra={'db_password': db_password,
                                         'redis_password': redis_password,
                                         'vuln_type': 'config_read'})
            # 配置文件读取成功但未提取到密码
            return ScanResult(kind='chain', name=self.name, status=STATUS_CONFIRMED,
                              severity=self.severity, url=url,
                              evidence='读取到配置文件但未提取到密码',
                              fix=self.fix,
                              extra={'db_password': '', 'redis_password': '',
                                     'vuln_type': 'config_read'})

        return ScanResult(kind='chain', name=self.name, status=STATUS_SAFE,
                          url=target, evidence='未读取到配置文件')
        ...
        if '"rows"' in text and ('"jobId"' in text or '"invokeTarget"' in text):
            ...
            if 'invokeTarget' in text:
                return ScanResult(kind='chain', name=self.name, severity=self.severity,
                                  ...
                                  fix=self.fix,
                                  extra={'job_endpoint': '/monitor/job',
                                         'vuln_type': 'job_rce'})
            # 仅未授权访问，无 RCE 特征
            return ScanResult(kind='chain', name=self.name, severity='medium',
                              ...
                              fix=self.fix,
                              extra={'job_endpoint': '/monitor/job',
                                     'vuln_type': 'job_unauth'})

        # 检查是否重定向到登录页（已鉴权）
        if 'login' in text.lower() or resp.status_code in (302, 401, 403):
            return ScanResult(kind='chain', name=self.name, status=STATUS_SAFE,
                              url=url, evidence='定时任务接口需鉴权')

        return ScanResult(kind='chain', name=self.name, status=STATUS_SAFE,
                          url=url, evidence='定时任务接口不可达或无特征')
