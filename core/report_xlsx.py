...
# 严重度整行填充色（ARGB）
_SEV_FILL = {
    SEVERITY_HIGH: PatternFill(start_color='FFF4CCCC', end_color='FFF4CCCC',
                                fill_type='solid'),  # 浅红
    SEVERITY_MEDIUM: PatternFill(start_color='FFFFF2CC', end_color='FFFFF2CC',
                                  fill_type='solid'),  # 浅黄
    SEVERITY_LOW: PatternFill(start_color='FFD9EAD3', end_color='FFD9EAD3',
                               fill_type='solid'),  # 浅绿
}
# 饼图分段颜色（高红/中黄/低绿）
_PIE_COLORS = ['CF222E', 'D4A72C', '1A7F37']
# 表头填充
_HEADER_FILL = PatternFill(start_color='FF0969DA', end_color='FF0969DA',
                            fill_type='solid')
_SUBHEADER_FILL = PatternFill(start_color='FF656D76', end_color='FF656D76',
                               fill_type='solid')
# 摘要标签列填充
_LABEL_FILL = PatternFill(start_color='FFF6F8FA', end_color='FFF6F8FA',
                           fill_type='solid')
# 状态中文名
_STATUS_CN = {
...
def _font(size=10, bold=False, color='FF1F2328', mono=False):
    """构造 Font 对象"""
    return Font(name=_FONT_MONO if mono else _FONT, size=size, bold=bold,
                color=color)
...
def _style_data_cell(cell, mono=False, bold=False, color='FF1F2328',
...
def _build_summary_sheet(ws, builder, gen_time):
    ...
    ws.cell(row=dist_start, column=1).font = Font(name=_FONT, size=12, bold=True,
                                                   color='FF1F2328')
    ...
    for j, (label, val) in enumerate(zip(dist_labels, dist_values), start=dist_start + 1):
        ...
        _style_data_cell(ws.cell(row=j, column=2), align='center',
                         bold=True, color='FF' + _PIE_COLORS[j - dist_start - 1])
    # 合计行
    ...
    if dr and dr.merged_groups > 0:
        ...
        ws.cell(row=dedup_row, column=1).font = Font(name=_FONT, size=12, bold=True,
                                                       color='FF1F2328')
    ...
    pie = PieChart()
    pie.title = '风险分布'
    labels_ref = Reference(ws, min_col=1, min_row=dist_start + 1,
                            max_row=dist_start + len(dist_labels))
    data_ref = Reference(ws, min_col=2, min_row=dist_start,
                          max_row=dist_start + len(dist_labels))
...
def _build_vuln_sheet(ws, builder):
    ...
    for i, r in enumerate(confirmed, start=2):
        ...
        row_data = [i - 1, r.name, sev_cn, status_cn, r.url or '',
                    r.evidence or '', r.fix or '', hit_count]
    ...
    if others:
        ...
        for i, r in enumerate(others, start=sep_row + 2):
            sev_cn = SEVERITY_CN.get(r.severity, r.severity)
            status_cn = _STATUS_CN.get(r.status, r.status)
            row_data = [i - sep_row - 1, r.name, sev_cn, status_cn,
                        r.url or '', r.evidence or '', '', '']
            for j, val in enumerate(row_data, start=1):
                cell = ws.cell(row=i, column=j, value=val)
                mono = j in (5, 6)
