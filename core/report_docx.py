...
def render_docx(builder, out_path):
    ...
    for i, (k, v) in enumerate(summary_rows):
        _add_cell_text(summary_table.rows[i].cells[0], k, size=10, bold=True, color=RGBColor(0x1F, 0x23, 0x28))
    ...
    for j, (h, v, f) in enumerate(zip(headers, values, fills)):
        ...
        _add_cell_text(dist_table.rows[1].cells[j], v, size=12, bold=True, align=WD_ALIGN_PARAGRAPH.CENTER)
    ...
    if not confirmed:
        ...
        for i, r in enumerate(confirmed, 1):
            ...
            _add_cell_text(vuln_table.rows[i].cells[0], r.name + hit_info, size=9, bold=True, color=name_color)
            _add_cell_text(vuln_table.rows[i].cells[1], sev_cn, size=9, align=WD_ALIGN_PARAGRAPH.CENTER)
    ...
    if confirmed:
        ...
        for i, r in enumerate(confirmed, 1):
            ...
            _set_run_font(fix_label, size=10, bold=True, color=RGBColor(0x1A, 0x7F, 0x37))
    ...
    footer_run = footer_para.add_run(f"由若依综合漏洞检测工具自动生成 · {gen_time} · 仅用于授权范围内的安全测试")
