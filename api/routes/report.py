...
def _find_report_file(task_id: str, fmt: str,
                      registry: TaskRegistry = None) -> str:
    """查找任务报告文件路径
...
async def get_report_metadata(task_id: str,
...
async def download_json_report(task_id: str,
                               registry: TaskRegistry = Depends(get_registry)):
    """下载 JSON 格式报告"""
...
async def download_csv_report(task_id: str,
    ...
    return FileResponse(filepath, media_type='text/csv',
                        filename=f'{task_id}_report.csv')
...
async def download_docx_report(task_id: str,
                               registry: TaskRegistry = Depends(get_registry)):
    """下载 Word 格式报告（D8 依赖）"""
...
async def download_xlsx_report(task_id: str,
                               registry: TaskRegistry = Depends(get_registry)):
    """下载 Excel 格式报告（D8 依赖）"""
